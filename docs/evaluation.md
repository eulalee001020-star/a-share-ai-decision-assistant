...
| 指标 | 合格标准 | 失败示例 |
| --- | --- | --- |
| 数据权限一致性 | 数据等级和动作权限一致 | 缺 A2 竞价却输出追强 |
...
演示时优先展示失败保护，而不是只展示漂亮输出：

1. 把 Demo 的 A2 数据关掉，说明系统如何禁止追强。
2. 把止损距离拉大，说明仓位上限如何下降。
3. 把失败概率调高，说明 expected R 如何转负并取消交易。
4. 展示 `prediction template`，说明如何把 AI 输出变成可校准数据。
