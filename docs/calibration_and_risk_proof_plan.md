...
> A1/B1 的公开数据健康规则已经完成过去一个月小样本校准；A2 竞价层因为公开历史数据不可得，不能被写成已校准，所以继续保留缺 A2 降级规则。
...
边界说明：当前验证不能外推为真实账户层面的长期收益提升或回撤下降。真实风险下降必须依赖持续行为日志和交易结果日志共同验证。
